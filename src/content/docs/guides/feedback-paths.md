...
To create feedback effects (such as echo, flanging, or resonant feedback), use the **Feedback - Volts** module (search for "feedback" — its category is Special/Feedback; a MIDI version, **Feedback - MIDI**, sits alongside it for feeding MIDI backward instead). This module allows backward routing by introducing a small delay of approximately 96 samples (~2 ms at 44.1 kHz).

### How It Works

1. Place a **Feedback - Volts** module in the feedback path
2. Route the signal from later in the chain back through Feedback - Volts to an earlier point
...
The **Delay** module has its own **Feedback** input that handles internal feedback without needing the Feedback - Volts module. Only use Feedback - Volts when routing signals backward between separate modules in the patch.
