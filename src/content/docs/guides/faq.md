...
That Wayland requirement covers the editor only. Plugins exported from SynthEdit work on Windows 7+ (VST) and macOS 10.15 Catalina or later (VST3 and Audio Unit, Intel and Apple Silicon). Linux export works too, though it's very new. A Linux plugin carries both display-server paths in one binary and picks whichever the host supports — Wayland embedding where it's offered (VST3 3.8.0+), X11 otherwise, which is what an X11 DAW like **Ardour** uses. One build covers both; test yours before shipping.
