...
- **Panel coordinates are not window-relative**: a container's panel content lives around the centre of a large virtual canvas (~3984), and its visible GUI window is the `panel_rect` (typically `[3732..4212]`). Asking for a control at "sensible" small coordinates like (200, 300) parks it in a far corner ~3700px from everything else, and the next panel screenshot spans both — a 4000px canvas with content in opposite corners that no crop can save. Place controls relative to a neighbour's `panelRect` from `se_dump`; `se_move` warns in its result when a panel move lands outside the visible window.
